...
// The puzzleInput provided by advent code.
const puzzleInput = `1640	590	93	958	73	1263	1405	1363	737	712	1501	390	68	1554	959	79
...
// A Row represents a single series of ordered numerical values
type Row []float64
...
func rowFromString(s string) (Row, error) {
	...
	r := make([]float64, len(fs))

	for i, f := range fs {
		v, err := strconv.ParseFloat(f, 64)
		if err != nil {
			return nil, errors.New("bad input: " + f + " cannot be parsed")
		}
		r[i] = v
	}
...
func findEvenDividend(r Row) float64 {
	...
	for i := range r {
		for j := i + 1; j < len(r); j++ {
			...
			if math.Mod(r[i], r[j]) == 0 {
				return r[i] / r[j]
			}

			if math.Mod(r[j], r[i]) == 0 {
				return r[j] / r[i]
			}
		}
	}
...
func spreadsheetChecksum(spreadsheet string) (checksum Checksum, err error) {
	...
	for s.Scan() {
		row, err := rowFromString(s.Text())
		if err != nil {
			return Checksum{}, err
		}

		checksum = update(row, checksum)
	}
...
func main() {
	c, err := spreadsheetChecksum(puzzleInput)
	if err != nil {
		log.Fatalf("Unable to parse spreadsheet checksum: %s", err)
	}

	log.Printf("Checksum: %+v", c)
}
